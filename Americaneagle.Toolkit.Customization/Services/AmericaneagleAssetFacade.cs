...
namespace Americaneagle.Toolkit.Customization.Services;


public class AmericaneagleAssetFacade(
        EntityIdentityFacade entityIdentityFacade,
        ToolConfiguration toolConfiguration,
        ModelFacade modelFacade,
        ContentFolderService contentFolderService,
        IImporter importer,
        ILogger<AmericaneagleAssetFacade> logger,
        IProtocol protocol
        ) : IAssetFacade
{
    ...
    public async Task<ContentItemSimplifiedModel> FromMediaFile(IMediaFile mediaFile, IMediaLibrary mediaLibrary, ICmsSite site, string[] contentLanguageNames)
    {
        ...
        var documentFileTypes = ".txt;.pdf;.docx;.pptx;.xlsx;.xml;.json";
        var targetDocumentFieldName = documentFileTypes.Contains(mediaFile.FileExtension, StringComparison.InvariantCultureIgnoreCase) ? "DocumentFile" : "ImageFile";
        var targetDocumentClassName = documentFileTypes.Contains(mediaFile.FileExtension, StringComparison.InvariantCultureIgnoreCase) ? "Accelerator.Document" : "Accelerator.Image";
        ...
        languageData.AddRange(contentLanguageNames.Select(contentLanguageName => new ContentItemLanguageData
        {
            ...
            {
                ["MediaTitle"] = mediaFile.FileName,
                ["MediaDescription"] = mediaFile.FileDescription,
                
                [targetDocumentFieldName] = new AssetFileSource
                {
        ...
        var contentItem = new ContentItemSimplifiedModel
        {
            ...
            IsSecured = null,
            ContentTypeName = targetDocumentClassName,
    ...
    public async Task<ContentItemSimplifiedModel> FromAttachment(ICmsAttachment attachment, ICmsSite site, ICmsTree? referencedNode, string[] contentLanguageNames)
    {
        ...
        var documentFileTypes = ".txt;.pdf;.docx;.pptx;.xlsx;.xml;.json";
        var targetDocumentFieldName = documentFileTypes.Contains(attachment.AttachmentExtension, StringComparison.InvariantCultureIgnoreCase) ? "DocumentFile" : "ImageFile";
        var targetDocumentClassName = documentFileTypes.Contains(attachment.AttachmentExtension, StringComparison.InvariantCultureIgnoreCase) ? "Accelerator.Document" : "Accelerator.Image";
        string mediaFolder = documentFileTypes.Contains(attachment.AttachmentExtension, StringComparison.InvariantCultureIgnoreCase) ? "Documents" : "Images";
        ...
        foreach (string contentLanguageName in contentLanguageNames)
        {
            ...
            var contentLanguageData = new ContentItemLanguageData
            {
                ...
                ContentItemData = new Dictionary<string, object?>
                {
                    ["MediaTitle"] = attachment.AttachmentName,
                    ["MediaDescription"] = attachment.AttachmentDescription,
                    [targetDocumentFieldName] = new AssetDataSource
                    {
    ...
    public string GetAssetUri(IMediaFile mediaFile, string? contentLanguageName = null)
    {
        ...
        return $"/getContentAsset/{ownerContentItemGuid}/c399619e-b8ff-4ee9-abe0-9c2cf4e89d46/{mediaFile.FileName}?language={contentLanguageSafe}";
    }
    ...
    public string GetAssetUri(ICmsAttachment attachment, string? contentLanguageName)
    {
        ...
        return $"/getContentAsset/{ownerContentItemGuid}/c204d644-0ccb-43d4-a505-69664b0a9bc3/{attachment.AttachmentName}?language={contentLanguageName ?? DefaultContentLanguage}";
    }
